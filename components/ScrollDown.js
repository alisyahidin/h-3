...
const ScrollDown = forwardRef(({ color: colorProps, onClick }, ref) => {
  const color = colorProps === 'dark' ? '#9D9D9D' : '#FFF'

  return (
    <div ref={ref} onClick={onClick} className="fixed left-0 bottom-0 ml-8 sm:ml-12 lg:ml-16 mb-16 pl-2 flex flex-col items-center hidden md:flex cursor-pointer">
      <p className="text-12px mb-4" style={{ color, writingMode: 'vertical-rl', transform: 'rotate(180deg)' }}>Scroll Down</p>
