...
const AwardCard = ({ data, sort, icon, onClick = null }) => {
  const image = icon
    ? process.env.NEXT_PUBLIC_API_URI + data.thumbnail.url
    : process.env.NEXT_PUBLIC_API_URI + data.picture.url
  ...
  return (
    ...
    >
      <img className="mb-4 mr-4 md:mr-8" src={image} alt={'Awward' + sort} style={{ maxWidth: 230 }} />
